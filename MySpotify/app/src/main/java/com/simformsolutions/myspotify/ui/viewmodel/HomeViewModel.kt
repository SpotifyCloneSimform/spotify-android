                    ...
                    is Resource.Success -> {
                        resource.data.let { data ->
                            val data = data?.items?.mapNotNull { item ->
                                item.images.firstOrNull()?.url?.let { HomeDisplayData(it, item.name ?: "") }
                            }
                            data?.let { data ->
            ...
            homeRepository.getSongAlbum().collectLatest { resource  ->
                when (resource) {
                    ...
                    is Resource.Success -> {
                        resource.data.let { data ->
                            val data = data?.items?.mapNotNull { item ->
                                item.album.images?.first()?.url?.let { HomeDisplayData(it, item.album.name ?: "N/A") }
                            }
                            data?.let { data ->
                    ...
                    is Resource.Success -> {
                        resource.data.let { data ->
                            val data = data?.playlists?.items?.mapNotNull { item ->
                                item.images.firstOrNull()?.url?.let { HomeDisplayData(it, item.name ?: "N/A") }
                            }
                            data?.let { data ->
                                val message = resource.data?.message
                                _playlists.emit(_playlists.value + HomeData( message ?: "Featured Playlist" , data))
                            }
                        }
                    }
